...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const entry = await runInTransaction(async session => {
    ...
    for (const row of rows) {
      ...
      const [batch] = await InventoryLot.create([{ item: item._id, stockEntry: entry._id, stockSerial, unitCostCents: costPriceCents, sellingPriceCents, expiryDate, originalQuantity: quantity, remainingQuantity: quantity }], { session });
...
router.get('/:id', asyncHandler(async (req, res) => {
  const data = await StockEntry.findById(req.params.id).populate('rows.batch').populate('createdBy', 'name');
